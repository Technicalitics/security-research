...
The `p` value used in the algorithm is prime. If we take a look at Fermat's Little Theorem/Euler's Criterion, they say:
x^{\frac{p-1}{2}} \equiv
\begin{cases}
1 \pmod{p} & \text{if } x \text{ is a quadratic residue mod } p,\\
-1 \pmod{p} & \text{if } x \text{ is a quadratic non-residue mod } p.
\end{cases}
...
**Step 1:** For each ciphertext element `c`, compute \( n^{\frac{p-1}{2}} \mod p \)
